...
const goNew = () => router.push("/simulation/run");
const goDetail = (id: number) => router.push(`/simulation/run?session_id=${id}`);
...
<template>
  <section class="card">
    <div class="header-row">
      <h2>沙盘推演 - 历史会话</h2>
      <button @click="goNew">新建模拟</button>
    </div>

    <p v-if="loading" class="tip">加载中...</p>
    <p v-else-if="!sessions.length" class="tip">暂无历史模拟，点击上方按钮创建</p>
    ...
    <div v-else class="grid-list">
      <div v-for="s in sessions" :key="s.id" class="item-card">
        ...
        <div class="item-body">
          <span class="label">平台</span> {{ s.platform }}
          <span class="label" style="margin-left:12px">轮次</span> {{ s.current_round }}/{{ s.total_rounds }}
        </div>
        <div class="item-meta">
        ...
        <div v-if="s.status === 'completed'" class="export-row">
          <button class="export-btn" @click="downloadReport('simulation', s.id, 'pdf')">PDF</button>
          <button class="export-btn" @click="downloadReport('simulation', s.id, 'pptx')">PPT</button>
        </div>
      </div>
    </div>
  </section>
...
.item-card {
  ...
  background: #f8fbff; transition: border-color 0.15s;
}

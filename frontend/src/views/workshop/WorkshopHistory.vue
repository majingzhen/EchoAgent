...
onMounted(async () => {
  loading.value = true;
  try {
    const resp = await getWorkshopSessions();
    ...
    <div v-else class="grid-list">
      <div v-for="s in sessions" :key="s.id" class="item-card">
        ...
        <div class="item-body">
          ...
          <span class="label" style="margin-left:12px">调性</span> {{ s.brand_tone }}
        </div>
        <div class="item-meta">
        ...
        <div v-if="s.status === 'completed'" class="export-row">
          <button class="export-btn" @click="downloadReport('workshop', s.id, 'pdf')">PDF</button>
          <button class="export-btn" @click="downloadReport('workshop', s.id, 'pptx')">PPT</button>
        </div>
      </div>
    </div>
  </section>
...
.item-card {
  border: 1px solid #d9e2ec; border-radius: 10px; padding: 14px; background: #f8fbff;
}

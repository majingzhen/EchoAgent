...
onMounted(async () => {
  loading.value = true;
  try {
    const resp = await getSentimentGuardSessions();
    sessions.value = resp.data ?? [];
  } catch { /* ignore */ }
  loading.value = false;
});

const goNew = () => router.push("/sentiment-guard/session");
const goDetail = (id: number) => router.push(`/sentiment-guard/session?session_id=${id}`);
...
<template>
  <section class="card">
    <div class="header-row">
      <h2>舆情哨兵 - 历史会话</h2>
      <button @click="goNew">新建评估</button>
    </div>
    ...
    <div v-else class="grid-list">
      <div v-for="s in sessions" :key="s.id" class="item-card">
        ...
        <div class="item-body">
          <span class="label">模式</span> {{ s.mode === "proactive" ? "事前预判" : "事后应对" }}
        </div>
        <div class="item-body desc">{{ (s.event_description ?? "").slice(0, 60) }}</div>
        <div class="item-meta">
        ...
        <div v-if="s.status === 'completed'" class="export-row">
          <button class="export-btn" @click="downloadReport('sentiment-guard', s.id, 'pdf')">PDF</button>
          <button class="export-btn" @click="downloadReport('sentiment-guard', s.id, 'pptx')">PPT</button>
        </div>
      </div>
    </div>
  </section>

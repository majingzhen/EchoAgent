...
用虚拟消费者画像替代昂贵的真实焦点小组，几分钟内获得用户洞察——Wynter / User Interviews 的开源平替。

[快速开始](#快速开始) · [功能演示](#功能模块) · [配置 LLM](#llm-配置) · [本地开发](#本地开发)
...
| 传统方式 | EchoAgent |
|---------|-----------|
| 焦点小组：$2000+，2-4 周 | 虚拟焦点小组：几分钟，零成本 |
| 用研报告：需专业研究员 | AI 自动生成结构化洞察 |
| 竞品分析：手工收集整理 | 上传资料，自动构建知识图谱 |
| 数据上传第三方 SaaS | 完全自托管，数据不出本地 |
...
| **内容工坊** | 多 Agent 流水线：策略 → 多版本文案 → 画像评分 → 品牌审核 |
...
- 任意兼容 OpenAI 接口的 LLM 服务（通义千问、OpenAI、Ollama、DeepSeek 等）
...
│   │   └── ws/           # WebSocket 管理
│   ├── config/
│   │   ├── app.yaml.example  # 配置模板（复制为 app.yaml 后填写）
│   │   └── app.yaml          # 本地配置（含 API Key，不入库）
│   └── migrations/
...
在开始之前，请阅读 [CONTRIBUTING.md](CONTRIBUTING.md)（即将发布）。
